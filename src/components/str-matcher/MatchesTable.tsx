...
const MatchesTable: React.FC<MatchesTableProps> = ({
 ...
 const filteredMatches = matches.filter(match => {
   try {
     console.log(`Filtering match ${match.profile.kitNumber}`);
     ...
     const markerMatch = Object.entries(markerFilters).every(([marker, isChecked]) => {
       ...
       if (!queryValue || !matchValue) {
         console.log(`Missing values for marker ${marker}`);
         return false;
       }
     ...
     const result = kitMatch && nameMatch && countryMatch && haploMatch && markerMatch;
     console.log(`Match ${match.profile.kitNumber} filtered:`, {
       kitMatch,
       nameMatch,
       countryMatch,
       haploMatch,
     ...
     console.error('Error filtering match:', error, match);
     return false;
   }
 });
 ...
 if (!matches.length || !query) {
   console.log('No matches or query to display');
   return null;
 }

 return (
   <div className="overflow-x-auto">
     <table className="min-w-full table-auto border-collapse text-sm matches-table">
       <thead>
               ...
               <div className="h-24">
                 <div className="absolute -rotate-90 origin-left whitespace-nowrap text-xs" style={{left: '50%', bottom: '8px'}}>
                   ...
                   <button 
                     ...
                     onClick={() => handleMarkerExclude(marker)}
                   >
                     ×
                   </button>
                 </div>
     ...
     {excludedMarkers.length > 0 && (
       <div className="mt-2 flex gap-2">
         <span>Исключенные маркеры:</span>
         {excludedMarkers.map(marker => (
           <button 
             key={marker}
             className="px-2 py-1 bg-blue-500 text-white rounded text-xs"
             onClick={() => handleMarkerInclude(marker)}
           >
             {marker} ↩
           </button>
         ))}
